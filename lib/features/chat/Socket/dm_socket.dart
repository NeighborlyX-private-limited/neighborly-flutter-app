...
class SocketService {
  ...
  void connect({String chatId = ''}) {
    ...
    _socket?.on("active-users", (userId) {
  ...
  void deleteMessage({required String chatId, required String messageId}) {
    print('DELETE MESSAGE WITH GROUP ID:$chatId AND MESSAGE ID $messageId');
    _socket?.emit(
      "delete-message",
      {
        "chatId": chatId,
  ...
  // JOIN ROOM EMITTER
  void joinRoom(String chatId) async {
    print('JOIN ROOM WITH GROUP ID:$chatId');
