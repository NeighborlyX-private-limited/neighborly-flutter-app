...
class ChatGroupCubit extends Cubit<ChatGroupState> {
  ...
  Future getGroupRoomMessages({bool? hideLoading = false, String? dateFrom}) async {
    print('... BLOC getGroupRoomMessages hideLoading=$hideLoading dateFrom=$dateFrom');
    if (!hideLoading!) {
      emit(state.copyWith(status: Status.loading));
    }
    final result = await getChatGroupRoomMessagesUseCase(roomId: state.roomId, dateFrom: dateFrom);
