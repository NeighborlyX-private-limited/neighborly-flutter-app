...
class _ReactionCommentWidgetState extends State<ReactionCommentWidget> {
  ...
  Future<void> _loadReactionState() async {
    setState(() {
      ...
      isCheered = ShardPrefHelper.getIsCheered(
              userID!, '${widget.comment.commentid}_isCheered') ??
          false;
      isBooled = ShardPrefHelper.getIsBoo(
              userID, '${widget.comment.commentid}_isBooled') ??
          false;
    });
  }
  ...
  Future<void> _saveReactionState() async {
    final userID = ShardPrefHelper.getUserID();
    ShardPrefHelper.setIsCheered(userID!, widget.comment.commentid, isCheered);
    ShardPrefHelper.setIsBoo(userID, widget.comment.commentid, isBooled);
  }

  // remove the reaction from shared preference
  Future<void> _removeReactionState() async {
    final userID = ShardPrefHelper.getUserID();
    ShardPrefHelper.setIsCheered(userID!, widget.comment.commentid, false);
    ShardPrefHelper.setIsBoo(userID, widget.comment.commentid, false);
  }
