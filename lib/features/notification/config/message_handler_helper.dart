...
class MessageHandlerHelper {
  ...
  void doTheJump() {
    ...
    print('message data NOW: $messageData');
    print('message data now: ${messageData['dmId'].runtimeType}');
    if (messageData['dmId'] != null && messageData['dmId'] != '') {
      print('is this alawas called');
      router.push(
        '/chat-private/${messageData['dmId']}',
        extra: {
          'profilePic': messageData['dmUserProfile'] ?? '',
          'userName': messageData['dmUserName'] ?? '',
          'isBack': true, // or false — optional
        },
      );
      return;
    }
    ...
    if (messageData['triggerType'] == 'AwardTrigger' &&
      ...
      router.push('/group-chat/${messageData['groupId']}?isBack=true');
